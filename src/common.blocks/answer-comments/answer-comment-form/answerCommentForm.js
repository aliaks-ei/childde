(function () {
  const commentFormActivator = document.querySelector(
    '[data-target="new-comment"]'
  );
  const commentFormCloseActivator = document.querySelector(
    '[data-target-close="new-comment"]'
  );
  const newCommentForm = document.getElementById("new-comment");
  const newCommentSendBtn =
    newCommentForm &&
    newCommentForm.querySelector(".new-comment-form__send-btn");

  function handleCommentTextareaChange(event) {
    const isSendBtnActive = Boolean(
  ...
  commentFormActivator &&
    commentFormActivator.addEventListener("click", function () {
      const textareaEl = newCommentForm.querySelector(
