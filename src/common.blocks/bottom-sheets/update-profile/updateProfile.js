(function () {
  ...
  const bsUpdateProfileActivators = document.querySelectorAll(
    '[data-target="bsUpdateProfile"]'
  );

  function handleBsUpdateProfileClick(event) {
    const currentMenuItem = event.target.closest(
      ".update-profile-menu__item-wrapper"
    );
