const answerModalActivators = document.querySelectorAll(
  '[data-target="answerModal"]'
);
const answerModalCloseActivators = document.querySelectorAll(
  '[data-target-close="answerModal"]'
);
...
function handleAnswerModalClick(event) {
  ...
  const isAnswerModalCloseActivatorClicked = [
    ...answerModalCloseActivators,
  ].some((activator) => activator.contains(event.target));
  ...
  if (uploadImageActivator.contains(event.target)) {
    ...
    isAnswerModalCloseActivatorClicked ||
    !this.firstElementChild.contains(event.target)
  ) {
    hideModal(this);

    this.removeEventListener("click", handleAnswerModalClick);
    uploadImageActivator.nextElementSibling.removeEventListener(
      "click",
      stopPropagationOnDropdown
    );
    addLinkActivator.nextElementSibling.removeEventListener(
      "click",
      stopPropagationOnDropdown
    );
    answerModal
      .querySelector("#answerModalContent")
      .removeEventListener("input", handleAnswerModalContentChange);
  }
}
...
function handleAnswerModalContentChange() {
  const symbolsCounter = answerModal.querySelector(
    ".symbols-counter-container"
  );
  const submitBtn = answerModal.querySelector(".answer-modal__answer-button");

  const svgFillCircle = symbolsCounter.querySelector(
    ".symbols-counter__fill-circle"
  );
  const svgBgCircle = symbolsCounter.querySelector(
    ".symbols-counter__bg-circle"
  );
  const symbolsCounterLabel = symbolsCounter.querySelector(
    ".symbols-counter__label__text"
  );
  const checkIcon = symbolsCounter.querySelector(
    ".symbols-counter__label .material-icons-round"
  );
...
for (const activator of answerModalActivators) {
  activator.addEventListener("click", () => {
    ...
    uploadImageActivator = answerModal.querySelector(
      '[data-target="uploadImageDropdown"]'
    );
    addLinkActivator = answerModal.querySelector(
      '[data-target="addLinkDropdown"]'
    );
    userSelectDropdownActivator = answerModal.querySelector(
      '[data-target="userSelectDropdown"]'
    );

    showModal(answerModal);

    answerModal.addEventListener("click", handleAnswerModalClick);
    uploadImageActivator.nextElementSibling.addEventListener(
      "click",
      stopPropagationOnDropdown
    );
    addLinkActivator.nextElementSibling.addEventListener(
      "click",
      stopPropagationOnDropdown
    );
    answerModal
      .querySelector("#answerModalContent")
      .addEventListener("input", handleAnswerModalContentChange);
  });
}
