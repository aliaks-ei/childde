...
const qThemesDropdownActivators = document.querySelectorAll(
  '[data-target="qThemesDropdown"]'
);
const qModalCloseActivators = document.querySelectorAll(
  '[data-target-close="qModal"]'
);
const qUploadCoverActivator = document.querySelector(
  '[data-target="qUploadCoverBtn"]'
);
const qDetailsActivators = document.querySelectorAll(
  '[data-target="qDetails"]'
);
const qThemesDropdownCloseActivators = document.querySelectorAll(
  '[data-target-close="qThemesDropdown"]'
);
const qCoverUploadCloseActivators = document.querySelectorAll(
  '[data-target-close="qCoverUpload"]'
);
...
function showQDetails() {
  qModalCover.insertAdjacentHTML(
    "afterend",
    `
    <textarea
...
function handleQModalClick(event) {
  ...
  const isQDetailsActivatorClicked = [...qDetailsActivators].some((activator) =>
  ...
  if (qCoverColors.contains(event.target)) {
    ...
    qModal
      .querySelector("#qModalTitle")
      .removeEventListener("input", handleQModalTitleChange);
  }
}
...
function handleQThemesDropdownClick(event) {
  ...
  const isQThemesDropdownCloseActivatorClicked = [
    ...qThemesDropdownCloseActivators,
...
function handleQUploadCoverClick(event) {
  ...
  const isQCoverUploadCloseActivatorClicked = [
    ...qCoverUploadCloseActivators,
  ].some((activator) => activator.contains(event.target));
...
function handleQModalTitleChange(event) {
  ...
  symbolsCounterLabel.textContent =
    currentSymbols <= -999 ? -999 : currentSymbols;
  svgFillCircle.style.strokeDashoffset = newStrokeDashoffset;

  submitBtn.disabled = Boolean(
    currentSymbols < 0 || currentSymbols == symbolsLimit
  );
}
...
for (const activator of qModalActivators) {
  activator.addEventListener("click", () => {
    ...
    userSelectDropdownActivator = qModal.querySelector(
    ...
    qModal
      .querySelector("#qModalTitle")
      .addEventListener("input", handleQModalTitleChange);
  });
}
