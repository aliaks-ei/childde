...
const hamburgerMenuActivator = document.querySelector(
...
function animateSignInLink(el, dashoffset) {
  ...
  if (dashoffset > 0) {
    setTimeout(
      () => animateSignInLink(el, dashoffset),
...
signInLink &&
  signInLink.addEventListener("mouseenter", () => {
    for (let line of signInLink.querySelectorAll(".sign-in-line")) {
      setTimeout(animateSignInLink(line, 22), 10);
    }
  });

hamburgerMenuActivator &&
  hamburgerMenuActivator.addEventListener("click", function () {
